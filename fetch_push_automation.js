	...
	setTimeout(() => {
		...
		readFile(
			...
			(err, data) => {
				...
				for (const week of contributions.data.user
					.contributionsCollection.contributionCalendar.weeks) {
					contributionFile.push(...week["contributionDays"]);
				}
				writeFile(
					`${workingDir}gh-contributions.json`,
		...
		setTimeout(() => {
			...
			git_stag.stdout.on("data", (data) => {
				console.log(`git_stag stdout: ${data}`);
			});
